...
interface ScrollableTextManagerProps {
  layers: TextLayer[]
  onChange: (layers: TextLayer[]) => void
}

export function ScrollableTextManager({ layers, onChange }: ScrollableTextManagerProps) {
  const addTextLayer = () => {
    ...
    <div className="space-y-4">
      ...
      <div className="max-h-96 overflow-y-auto space-y-3 pr-2">
        {layers.map((layer, index) => (
          <Card key={layer.id} className="border-2 border-gray-200 dark:border-gray-700 rounded-xl">
            <CardHeader className="pb-2">
              <div className="flex items-center justify-between">
                <CardTitle className="text-sm font-medium">Text Layer {index + 1}</CardTitle>
                <div className="flex items-center space-x-1">
                  ...
                  {layers.length > 1 && (
                    <Button
                      ...
                      className="h-6 w-6 text-red-500 hover:text-red-700 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg"
                    >
                      <X className="h-3 w-3" />
                    </Button>
                  )}
            ...
            <CardContent className="space-y-3">
              ...
              <div className="space-y-2">
                ...
                <div className="flex space-x-2">
                  <Input
                    ...
                    className="w-16 h-10 p-1 border-2 rounded-lg cursor-pointer"
                  />
                  <Input
                    type="text"
                    value={layer.color}
              ...
              <div className="space-y-2">
                <Label className="text-xs font-medium">Font Size: {layer.size}px</Label>
                <Slider
                  value={[layer.size]}
              ...
              <div className="space-y-2">
                <Label className="text-xs font-medium">Clockwise Rotation: {layer.rotation}°</Label>
                <Slider
